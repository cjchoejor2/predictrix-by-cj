...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function syncOtherPanels(currentPanelId, symbol) {
    ...
    symbolSelects.forEach(select => {
      ...
      if (select.value.toUpperCase() === symbol) {
        ...
        const fetchBtn = document.getElementById(`fetchData${panelId}`);
        if (fetchBtn) {
          // Increase delay for each panel to avoid rate limits
          delay += 2000; // 2 second delay between requests
          
          // Trigger analysis after a delay to avoid rate limiting
          setTimeout(() => {
  ...
  // Auto-analyze only the first panel on load to avoid rate limiting
  setTimeout(() => {
    const firstBtn = document.querySelector('[id="fetchData"]');
    if (firstBtn) {
      analyzeMarket('');
    }
  }, 500);
